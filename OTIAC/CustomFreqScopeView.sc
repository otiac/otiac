CustomFreqScopeView {
	...
	*initSynthDefs {
		...
		// linear
		SynthDef("system_CP_freqScope0", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02;
		...
		SynthDef("system_CP_freqScope0_shm", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, numSamples, mul;
			var fftbufnum = LocalBuf(fftBufSize, 1);
			mul = 0.00285;
		...
		// logarithmic
		SynthDef("system_CP_freqScope1", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02;
			var phase = 1 - (rate * fftBufSize.reciprocal);
		...
		SynthDef("system_CP_freqScope1_shm", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, halfSamples, mul;
			var fftbufnum = LocalBuf(fftBufSize, 1);
			mul = 0.00285;
		...
		SynthDef("system_CP_freqScope0_magresponse", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02, in2=1;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, numSamples, mul;
		...
		SynthDef("system_CP_freqScope0_magresponse_shm", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02, in2=1;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, numSamples, mul;
			var signal2, chain2, divisionbuf;
			var fftbufnum = LocalBuf(fftBufSize, 1);
		...
		SynthDef("system_CP_freqScope1_magresponse", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02, in2=1;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, halfSamples, mul;
			var signal2, chain2, divisionbuf;
			var fftbufnum = LocalBuf(fftBufSize, 1);
		...
		SynthDef("system_CP_freqScope1_magresponse_shm", { arg in=0, fftBufSize = 2048, scopebufnum=1, rate=4, dbFactor = 0.02, in2=1;
			var phase = 1 - (rate * fftBufSize.reciprocal);
			var signal, chain, phasor, halfSamples, mul;
			var signal2, chain2, divisionbuf;
			var fftbufnum = LocalBuf(fftBufSize, 1);
	...
	initFreqScope { arg parent, bounds, argServer;
		...
		if (this.shmScopeAvailable) {
			...
			scope.waveColors_([Color.black]);
			scope.background_(Color.new255(230, 230, 230));
			scope.server = server;
			scope.fill = false;
		} {
	...
	start {
		...
		defname = specialSynthDef ?? {
			"system_CP_freqScope" ++ freqMode.asString ++ if (this.shmScopeAvailable) {"_shm"} {""}
		};
	...
	*response { |parent, bounds, bus1, bus2, freqMode = 1|
		...
		var synthDefName = "system_CP_freqScope%_magresponse%".format(freqMode, if (scope.shmScopeAvailable) {"_shm"} {""});
...
CP_FreqScope {
	...
	*new { arg width=522, height=300, busNum=0, scopeColor, bgColor, server;
		...
		if(scopeOpen != true, { // block the stacking up of scope windows
			...
			scope = CustomFreqScopeView(window, rect.moveBy(pad[0], pad[2]), server);
